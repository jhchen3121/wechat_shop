...
class Handler(handler.handler):
    ...
    def dispatch(self, session):
        ...
        session.query(WechatshopOrderGood).filter(WechatshopOrderGood.id == id).update({WechatshopOrderGood.is_delete: 1})
        session.commit()
        ...
        session.query(WechatshopOrder).filter(WechatshopOrder.id == order_id).update({WechatshopOrder.actual_price: actual_price, WechatshopOrder.order_price: order_price, WechatshopOrder.goods_price: goods_price})
        session.commit()
