    ...
    export default {
        ...
        methods: {
            ...
            changeShowStatus($event, para) {
                this.axios.post(this.root + 'category/change_show_status', {
                    params: {
                        status: $event,
                        id: para
                    }
            ...
            changeChannelStatus($event, para) {
                this.axios.post(this.root + 'category/change_channel_status', {
                    params: {
                        status: $event,
                        id: para
                    }
            ...
            changeCategoryStatus($event, para) {
                this.axios.post(this.root + 'category/change_category_status', {
                    params: {
                        status: $event,
                        id: para
                    }
            ...
            submitSort(index, row){
                this.axios.post(this.root + 'category/update_cate_sort', { id: row.id,sort:row.sort_order }).then((response) => {
                })
            },
            ...
            specDelete(index, row) {
                this.$confirm('确定要删除?', '提示', {
                    ...
                    this.axios.post(this.root + 'specification/delete', {id: row.id}).then((response) => {
            ...
            handleRowDelete(index, row) {
                this.$confirm('确定要删除?', '提示', {
                    ...
                    this.axios.post(this.root + 'category/delete_category', {id: row.id}).then((response) => {
                        if (response.data.header.code === 0) {
                            this.$message({
                                type: 'success',
                                message: '删除成功!'
            ...
            getList() {
                this.axios.post(this.root + 'category/get_list', {
                    params: {
                        page: this.page,
                    }
                }).then((response) => {
                    this.categoryData = response.data.body.data
                })
            },
            getSpecList() {
                this.axios.post(this.root + 'specification/index').then((response) => {
                    this.specData = response.data.body.data
                })
            }
        },
        components: {},
