...
<script>
    ...
    export default {
        data() {
            return {
                myHeaders: {'x-session-token': sessionStorage.getItem('token')},
                root: '',
                qiniuZone:'',
                picData: {
                    token: ''
                ...
                cateOptions: [],
                uploaderHeader: {
                    'X-Nideshop-Token': sessionStorage.getItem('token') || '',
                },
                editorOption: {
                    modules: {
                        toolbar: {
        ...
        methods: {
            ...
            checkSkuOnly(index,row){
                ...
                this.axios.post(this.root + 'goods/check_sku', {info: row}).then((response) => {
                    if (response.data.header.code === -100) {
                        this.$message({
                            type: 'error',
                            message: '该SKU已存在！'
                        })
            ...
            getSpecData() {
                ...
                this.axios.post(this.root + 'specification/get_goods_spec', {id: id}).then((response) => {
                    if (response.data.header.code === 0) {
                        let info = response.data.body.data;
            ...
            delePicList() {
                ...
                let id = this.infoForm.id;
                this.$confirm('确定删除首图,删除后无法恢复？', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.axios.post(this.root + 'goods/delete_list_pic', {id: id}).then((response) => {
                        if (response.data.header.code === 0) {
                            this.$message({
                                type: 'success',
                                message: '删除成功'
            ...
            galleryRemove(file, fileList) {
                ...
                this.axios.post(this.root + 'goods/delete_gallery_file', para).then((response) => {
                    if (response.data.header.code === 0) {
                        this.$message({
                            type: 'success',
                            message: '删除成功'
            ...
            getGalleryList() {
                ...
                this.axios.post(this.root + 'goods/get_gallery_list', {goodsId: goodsId}
                ).then((response) => {
                    this.gallery_list = response.data.body.data;
                })
            },
            kdChange(kdValue) {
            ...
            onCopyGood() {
                this.$confirm('确定复制该商品？', '提示', {
                    ...
                    this.axios.post(this.root + 'goods/copy_goods', {id: this.infoForm.id}).then((response) => {
                        if (response.data.header.code === 0) {
                            this.$message({
                                type: 'success',
                                message: '复制成功!'
...
//                            this.specData = [];
            ...
            onSubmitInfo() {
                this.$refs['infoForm'].validate((valid) => {
                    if (valid) {
                        ...
                        // return false;
                        this.axios.post(this.root + 'goods/cu_goods',
                            {
                                info: this.infoForm,
            ...
            handleUploadListSuccess(res) {
                let img_url = api.baseUrl + '/static_source/' + res.body.files[0].ref_path;
                this.infoForm.list_pic_url = img_url;
            ...
            handleUploadIndexPicSuccess(res) {
                ...
                let img_url = api.baseUrl + '/static_source/' + res.body.files[0].ref_path;
                this.infoForm.index_pic_url = img_url;
            },
            ...
            handleUploadGallerySuccess(res) {
                //console.log(res);
                //let url = this.url;
                // 轮播图必须在生成商品之后再插入，否则无id
                if (res.key != '') {
                    //let urlData = url + res.key;
                    let urlData = api.baseUrl + '/static_source/' + res.body.files[0].ref_path;
                    let id = this.infoForm.id;
                    let info = {
                        url: urlData,
                        goods_id: id
                    ...
                    let that = this
                    this.axios.post(this.root + 'goods/save_gallery', info).then((response) => {
                        that.getGalleryList();
                    })
                }
            },
            ...
            getInfo() {
                ...
                //加载商品详情
                let that = this
                this.axios.post(this.root + 'goods/info', {
                    params: {
                        id: that.infoForm.id
                    }
                }).then((response) => {
                    let resInfo = response.data.body.info_data;
            ...
            getAllCategory() {
                let that = this;
                this.axios.post(this.root + 'goods/get_all_category', {
                    params: {}
                }).then((response) => {
                    that.options = response.data.body.data;
                })
            },
            getAllSpecification() {
                let that = this;
                this.axios.post(this.root + 'goods/get_all_specification').then((response) => {
                    let resInfo = response.data.body.data;
                    that.specOptionsList = resInfo;
                })
            },
            getExpressData() {
                let that = this
                this.axios.post(this.root + 'goods/get_express_data', {
                    params: {}
                }).then((response) => {
                    let options = response.data.body.data;
