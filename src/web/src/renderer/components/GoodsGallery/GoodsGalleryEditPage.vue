...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            onSubmitInfo() {
                this.axios.post(this.root + 'goods/change_gallery_sort', this.infoForm).then((response) => {
...
//                        this.$router.go(-1);
            ...
            getInfo() {
                ...
                //加载广告详情
                let that = this
                this.axios.post(this.root + 'goods/get_gallery_detail', {
                    params: {
                        id: that.infoForm.id
                    }
                }).then((response) => {
                    that.infoForm = response.data.body;
                })
            }
        }
        ,
