...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            relateGoodsClick() {
                this.axios.post(this.root + 'shop_set/get_all_ad_goods', {id: this.infoForm.id}).then((response) => {
                    if (response.data.header.code === 0) {
                        this.chooseRelateGoods = response.data.body.data
                    }
                });
            },
            test() {
            ...
            onSubmitInfo() {
                ...
                this.$refs['infoForm'].validate((valid) => {
                    if (valid) {
                        this.axios.post(this.root + 'shop_set/cu_ad', this.infoForm).then((response) => {
                            if (response.data.header.code === 0) {
                                this.$message({
                                    type: 'success',
                                    message: '保存成功'
                                ...
                                this.$router.go(-1);
                            } else if (response.data.header.code === -100) {
                                this.$message({
                                    type: 'error',
                                    message: '该商品已经有广告关联'
                                })
            ...
            handleUploadImageSuccess(res, file) {
                ...
                this.infoForm.image_url = img_url;
            },
            getInfo() {
                ...
                //加载广告详情
                let that = this
                this.axios.post(this.root + 'shop_set/get_ad_info', {
                    params: {
                        id: that.infoForm.id
                    }
                }).then((response) => {
