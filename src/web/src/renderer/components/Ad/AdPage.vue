...
export default {
	...
	methods: {
        ...
        submitSort(index, row){
            this.axios.post(this.root + 'shop_set/update_ad_sort', { id: row.id,sort:row.sort_order }).then((response) => {
            })
        },
        changeStatus($event, para) {
            this.axios.post(this.root + 'shop_set/change_ad_status', {
                params: {
                    status: $event,
                    id: para
                }
		...
		handlePageChange(val) {
			this.page = val;
			//保存到sessionStorage
			sessionStorage.setItem('adPage', this.page)
			sessionStorage.setItem('adFilterForm', JSON.stringify(this.filterForm));
			this.getList()
		},
		...
		handleRowDelete(index, row) {
			...
			this.$confirm('确定要删除?', '提示', {
				...
				this.axios.post(this.root + 'shop_set/delete_ad', { id: row.id }).then((response) => {
		...
		getList() {
			this.axios.post(this.root + 'shop_set/show_ad', {
				params: {
					page: this.page,
				}
			}).then((response) => {
