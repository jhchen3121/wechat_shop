        ...
        methods: {
            ...
            onSubmitInfo() {
                ...
                this.$refs['infoForm'].validate((valid) => {
                    // valid存在bug验证，手动if
                    if (this.infoForm.name && this.infoForm.front_name && this.infoForm.icon_url && this.infoForm.img_url) {
                        this.axios.post(this.root + 'category/cu_cate', this.infoForm).then((response) => {
            ...
            handleUploadBannerSuccess(res, file) {
                ...
                this.infoForm.img_url = img_url;
            },
            handleUploadIconSuccess(res, file) {
                let icon_url = api.baseUrl + "/static_source/" + res.body.files[0].ref_path;
                this.infoForm.icon_url = icon_url;
            },
            getTopCategory() {
                this.axios.get(this.root + 'category/get_top_cate').then((response) => {
                    this.parentCategory = this.parentCategory.concat(response.data.body.data);
                })
            },
            getInfo() {
                ...
                //加载分类详情
                let that = this
                this.axios.post(this.root + 'category/get_info', {
                    params: {
                        id: that.infoForm.id
                    }
                }).then((response) => {
